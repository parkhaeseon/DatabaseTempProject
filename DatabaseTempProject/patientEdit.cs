...
namespace DatabaseTeamProject
{
    ...
    {
        public pEditSearch patEdit;

        public patientEditAddress pea;

        dba a = new dba();

        public patientEdit(pEditSearch _patEdit)
        {
            InitializeComponent();

            this.patEdit = _patEdit;
        ...
        private void myinforedit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                string emp_update = "update patient set "
                                    ...
                                    + "where id = '" + patEdit.patId.Text.ToString().Trim() + "'";

                MySqlCommand cmd = new MySqlCommand(emp_update, a.Connection());

                cmd.ExecuteNonQuery();

                MessageBox.Show("환자 개인정보가 수정되었습니다.", "환자 개인정보 수정 완료", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        {
            pea = new patientEditAddress(this);
            pea.ShowDialog();
        }
